...
class Classes implements SchoolStoreInterface {
  ...
  @action updateSchool = async (schoolUpdate: ISchool) => {
    try {
        console.log("schoolUpdate...",schoolUpdate);
      const classe = await axios.patch(`${config.servers.apiUrl}classes/edit`, schoolUpdate);
